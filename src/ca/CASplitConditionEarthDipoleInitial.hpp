class CASplitConditionEarthDipoleInitial: public CASplitCondition{
	...
	public: bool doSplit(Cell* cell){
		...
		double r[3];
		double R = td->getLowBorderRadius();
...
//		if(hcell->getRadius() < 4 * Constants::Re && minH > 0.125 * Constants::Re) return true;
//		if(hcell->getRadius() < 3.5 * Constants::Re && minH > 0.0625 * Constants::Re) return true;
