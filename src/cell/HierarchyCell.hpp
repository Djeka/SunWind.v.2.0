...
////////////////////////
	...
	public: void merge(){
		...
		interpolateValuesForMerge();
		this->setIsSplitted(false);

		for(int icell=0; icell<8; icell++){
			HierarchyCell* subCell = getSubCell(icell);
			subCell->merge();
		}

		for(int icell=0; icell<8; icell++) mergeSidesForSubCell(getSubCell(icell));
		for(int icell=0; icell<8; icell++) mergeRibsForSubCell(getSubCell(icell));
		for(int icell=0; icell<8; icell++){
			delete getSubCell(icell);
			setSubCell(icell, NULL);
		}
	}

	private: void interpolateValuesForMerge(){
		setDensity(getDensity());
		for(int axis=0; axis<3; axis++){
			setMomentum(getMomentum(axis), axis);
			for(int dir=0; dir<2; dir++){
				getSide(axis, dir)->setB(getSide(axis, dir)->getB());
			}
			for(int irib=0; irib<4; irib++){
				getRib(axis, irib)->setE(getRib(axis, irib)->getE());
			}
		}
		setP(getP());
	}

	private: void mergeSidesForSubCell(HierarchyCell* cell){
		for(int axis=0; axis<3; axis++){
			for(int dir=0; dir<2; dir++){
				HierarchySide* side = (HierarchySide*) cell->getSide(axis, dir);
				if(side == NULL) continue;

				HierarchyCell* fcell = (HierarchyCell*) side->getFCell();
				HierarchyCell* bcell = (HierarchyCell*) side->getBCell();

				if(fcell != NULL && bcell != NULL && fcell->getParent() == bcell->getParent()){
					side->free();
				} else if(fcell != NULL && bcell != NULL && fcell->getDeep() == bcell->getDeep()){
					setCellForSideAndAllSubSides(side, 1-dir, cell->getParent());
				} else if((fcell != NULL && bcell != NULL && fcell->getDeep() != bcell->getDeep()) ||
						(fcell != NULL && bcell == NULL) || (fcell == NULL && bcell != NULL)){
					if(cell->getPoint((axis+1)%3) == 0 && cell->getPoint((axis+2)%3) == 0){
						side->getRib(Side::RIGHT_RIB)->setSide(Rib::DOWN_SIDE, NULL);
						side->getRib(Side::LEFT_RIB)->setSide(Rib::UP_SIDE, side->getParent());
						side->getRib(Side::UP_RIB)->setSide(Rib::LEFT_SIDE, NULL);
						side->getRib(Side::DOWN_RIB)->setSide(Rib::RIGHT_SIDE, side->getParent());
					} else if(cell->getPoint((axis+1)%3) == 1 && cell->getPoint((axis+2)%3) == 0){
						side->getRib(Side::RIGHT_RIB)->setSide(Rib::DOWN_SIDE, side->getParent());
						side->getRib(Side::LEFT_RIB)->setSide(Rib::UP_SIDE, NULL);
						side->getRib(Side::UP_RIB)->setSide(Rib::LEFT_SIDE, NULL);
						side->getRib(Side::DOWN_RIB)->setSide(Rib::RIGHT_SIDE, side->getParent());
					} else if(cell->getPoint((axis+1)%3) == 0 && cell->getPoint((axis+2)%3) == 1){
						side->getRib(Side::RIGHT_RIB)->setSide(Rib::DOWN_SIDE, NULL);
						side->getRib(Side::LEFT_RIB)->setSide(Rib::UP_SIDE, side->getParent());
						side->getRib(Side::UP_RIB)->setSide(Rib::LEFT_SIDE, side->getParent());
						side->getRib(Side::DOWN_RIB)->setSide(Rib::RIGHT_SIDE, NULL);
					} else if(cell->getPoint((axis+1)%3) == 1 && cell->getPoint((axis+2)%3) == 1){
						side->getRib(Side::RIGHT_RIB)->setSide(Rib::DOWN_SIDE, side->getParent());
						side->getRib(Side::LEFT_RIB)->setSide(Rib::UP_SIDE, NULL);
						side->getRib(Side::UP_RIB)->setSide(Rib::LEFT_SIDE, side->getParent());
						side->getRib(Side::DOWN_RIB)->setSide(Rib::RIGHT_SIDE, NULL);
					}
					side->getParent()->setIsSplitted(false);
					side->free();
				}
			}
		}
	...
	private: void setCellForSideAndAllSubSides(HierarchySide* side, int dir, Cell* cell){
		side->setCell(dir, cell);
		if(!side->isSplitted()) return;

		for(int iside=0; iside<4; iside++){
			HierarchySide* subside = side->getSubSide(iside);
			setCellForSideAndAllSubSides(subside, dir, cell);
		}
	}

	private: void mergeRibsForSubCell(HierarchyCell* cell){
		for(int axis=0; axis<3; axis++){
			for(int irib=0; irib<4; irib++){
				HierarchyRib* rib = (HierarchyRib*) cell->getRib(axis, irib);
				if(rib == NULL) continue;

				if(doFreeRib(rib, cell)){
					if(rib->getParent() != NULL) rib->getParent()->setIsSplitted(false);
					rib->free();
					continue;
				}

				for(int icell=0; icell<4; icell++){
					if(cell == rib->getCell(icell)){
						setCellForRibAndALlSubRibs(rib, icell, cell->getParent());
					}
				}
			}
		}
	...
	private: void setCellForRibAndALlSubRibs(HierarchyRib* rib, int ind, Cell* cell){
		rib->setCell(ind, cell);
		if(!rib->isSplitted()) return;

		for(int irib=0; irib<2; irib++){
			HierarchyRib* subrib = rib->getSubRib(irib);
			setCellForRibAndALlSubRibs(subrib, ind, cell);
		}
	}

	private: bool doFreeRib(Rib* rib, HierarchyCell* cell){
		for(int iside=0; iside<4; iside++){
			HierarchySide* side = (HierarchySide*) rib->getSide(iside);
//cout << "\t\t\t" << side << endl;
			if(side != NULL && getSideDeep(side) == cell->getDeep())
				return false;
		}
	...
	private: int getSideDeep(HierarchySide* side){
		HierarchyCell* fcell = (HierarchyCell*) side->getFCell();
		HierarchyCell* bcell = (HierarchyCell*) side->getBCell();

		if(fcell == NULL && bcell != NULL) return bcell->getDeep();
		if(fcell != NULL && bcell == NULL) return fcell->getDeep();

		return max(fcell->getDeep(), bcell->getDeep());
	}
};
