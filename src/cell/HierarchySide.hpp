...
class HierarchySide : public Side
{
	...
	public: void free(){
		...
		if(bcell != NULL && bcell->getSide(axis, 1) == this) bcell->setSide(axis, 1, NULL);
	...
	public: double getEqFlow(int ind){
		if(isSplitted()){
			eqF[ind] = 0;
			for(int i=0; i<4; i++)	eqF[ind] += 0.25 * subsides[i]->getEqFlow(ind);
			return  eqF[ind];
		} else{
			return eqF[ind];
		}
