class InitialParametersFunctionEarth: public InitialParametersFunction{
	private: TaskData* td;
	private: static const double T0 = 600.0;
	...
	public: double getPressure(double* r){
		return (Constants::gamma - 1.0) * getDensity(r) * Constants::Cv * T0;
	};
...
/*		int axe1 = (axis+1)%3;
		...
		return B;
*/
		return getDipoleField(r0)[axis];
	}

	private: vector<double> vectorpotential(double r[], double P[]){
		vector<double> A(3);
		double r3 = pow(Utils::csqrt(r[0], r[1], r[2]), 3);
	...
	public: double getMaxSpeed(double r[]){
		...
		double maxs = 0;
		vector<double> B = getDipoleField(r);
	...
	private: vector<double> getDipoleField(double r[]){
		double p[3], rd[3];
		vector<double> B(3);
		p[0] = 0;
		p[1] = 0;
		...
		double rabs = Utils::csqrt(r[0], r[1], r[2]);
